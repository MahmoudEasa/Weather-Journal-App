...
const apiKey = "&appid=21d44e5ccb081708196324f2b062ea34";
...
// Create an if condition to check if user enters a value or not
    ...
    .then(res =>{
        console.log(res);
        postData("/add", {date:newDate, temp:res.list[0].main.temp, content:newFeelings});
    })
    // Then Update UI
    .then(res =>{
        updateUI()
...
const updateUI = async ()=>{
    ...
    try{
        ...
        document.querySelector("#date").innerHTML = `Date Is: ${allData[0].date}`;
        document.querySelector("#temp").innerHTML = `Temperature Is: ${allData[0].temp}`;
        document.querySelector("#content").innerHTML = `I feel: ${allData[0].content}`;
    }catch(error){
        console.log("error", error);
    };
};
